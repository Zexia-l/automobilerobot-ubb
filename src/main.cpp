...
void setup_imu() { // FUNGSI BARU UNTUK SETUP IMU
    ...
    SerialUSB.println("--- Setup IMU ---");
    // Cek koneksi
    if (!mag.testConnection()) {
        SerialUSB.println("FATAL: HMC5883L failed.");
    }
    if (!accelgyro.testConnection()) {
        SerialUSB.println("FATAL: MPU6050 failed.");
    }
...
void setup_ros_messages() {
    ...
    for(int i = 0; i < NUM_MOTORS; i++) {
        if (!rosidl_runtime_c__String__assign(&joint_state_msg.name.data[i], joint_names[i])) {
            SerialUSB.println("FATAL: Gagal alokasi nama joint.");
            while(1);
        }
    }
...
void setup() {
    ...
    SerialUSB.println("--- Setup ROS Selesai ---");
}
